...
def layout(usina):
    if not st.session_state.load_data:
        if 'periodo' in st.session_state:
            from libs.models.datas import  get_periodo
    ...
    with col2:
        ...
        with col2_1:
            valor_Mwh = st.number_input('Valor do MWh R$', value=250.00, format='%0.2f')
        with col2_2:
            percentual = st.number_input('Participação %', value=100.00,  min_value=0.00, max_value=100.00, format='%0.2f')
        for i, (key, value) in enumerate(st.session_state.list_cards.items()):
            value['valor_real'] = round(float(value['value']) * valor_Mwh * (percentual/100), 2)
            create_energy_card(key, value['value'], value['data_hora'], value['medida'], value['percentual'], value['value_max'], value['value_min'], value['valor_real'])
